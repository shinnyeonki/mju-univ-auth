# 명지대학교 통합 인증 및 서비스 로그인 과정 분석

## 1. 개요

이 문서는 명지대학교 학생 정보 시스템(MSI)에 로그인하여 학생 정보를 조회하는 전체 과정을 심층적으로 분석하고 설명합니다.

명지대학교의 통합 로그인(SSO) 시스템은 보안을 위해 복잡한 인증 절차를 사용합니다. 이 과정은 단순히 아이디와 비밀번호를 전송하는 것을 넘어, 하이브리드 암호화(RSA+AES), CSRF 토큰, 여러 단계의 리다이렉션 및 자동 폼 제출을 포함합니다. 이 모든 과정을 자동화해야 최종적으로 학생 정보를 추출할 수 있습니다.

본 문서는 다음 핵심 프로세스를 중심으로 설명합니다.

1.  **SSO 통합 로그인 과정**: 암호화에 필요한 정보를 서버로부터 받고, 자격 증명을 안전하게 암호화하여 전송하며, 최종적으로 모든 서비스에서 사용 가능한 세션을 획득하는 과정을 설명합니다.
2.  **학생카드 정보 조회 과정**: 획득한 세션을 사용하여 학생카드 페이지에 접근하고, 2차 비밀번호 인증을 거쳐, 최종적으로 HTML 페이지에서 학생 정보를 파싱하는 과정을 설명합니다.
3.  **학적변동내역 조회 과정**: 획득한 세션을 사용하여 학적변동내역 페이지에 접근하고, HTML 페이지에서 정보를 파싱하는 과정을 설명합니다.

## 2. 핵심 동작 원리: 클라이언트-서버 상호작용

서버와의 모든 통신은 HTTP 세션(Session)을 통해 이루어집니다. 세션은 여러 요청에 걸쳐 쿠키를 자동으로 관리하여 서버와의 연속적인 통신 상태를 유지하는 역할을 합니다.
...
-   **Client Action**: 프로세스는 목표 서비스(`msi`)에 해당하는 SSO URL로 `GET` 요청을 보내는 것으로 시작됩니다.
    -   **Method**: `GET`
...
-   **Client Action (Parsing)**: 클라이언트는 응답받은 HTML을 파싱하여 다음 정보를 추출합니다.
...
#### **2단계: 클라이언트 측 하이브리드 암호화**

서버로 비밀번호를 직접 보내는 대신, 실제 웹사이트의 JavaScript 코드 동작을 그대로 모방한 하이브리드 암호화 방식을 사용합니다. 이는 대칭키 암호화(AES)의 속도와 비대칭키 암호화(RSA)의 보안성을 결합한 것입니다.

1.  **세션키 생성**:
    -   먼저, 일회용 대칭키로 사용할 **세션키(Session Key)**를 생성합니다.
    -   64바이트의 랜덤 데이터를 생성합니다.
...
2.  **키 교환: 세션키 암호화**:
    ...
    -   암호화된 결과는 Base64로 인ко딩되어 `encsymka`라는 이름의 필드에 담깁니다.
...
-   **Client Action**: 클라이언트는 2단계에서 준비된 암호화 데이터를 `POST` 요청의 본문(form data)에 담아 1단계에서 추출한 `form_action` URL로 전송합니다.
    -   **Method**: `POST`
...
인증에 성공하면, 서버는 사용자를 원래 요청했던 서비스(MSI)로 보내주기 위해 여러 번의 리다이렉션을 수행합니다. HTTP 클라이언트는 이 과정을 자동으로 처리해야 합니다.
...
-   **Server Response (Type 1: HTTP 302 Redirect)**
    ...
    -   클라이언트는 이 `Location` 헤더를 자동으로 따라가 다음 요청을 보냅니다.
...
-   **Server Response (Type 2: JavaScript 자동 폼 제출)**
    -   일부 페이지는 HTTP 리다이렉션 대신, 페이지 로드 시(`onLoad`) JavaScript로 폼을 자동 제출하는 방식을 사용합니다.
        ...
        ```
    -   **Client Action**:
        -   클라이언트는 응답받은 HTML에 자동 폼 제출 관련 코드가 있는지 감지해야 합니다.
        -   HTML을 파싱하여 `<form>` 태그의 `action` URL과 내부 `<input>` 필드들을 추출합니다.
...
-   **Server Response (Type 3: JavaScript `location.href` 리다이렉트)**
    ...
    -   **Client Action**: 클라이언트는 `location.href` 패턴을 감지하고, 지정된 URL로 `GET` 요청을 보내 리다이렉션을 수동으로 처리합니다.

-   **Final Result**:
    -   이 모든 리다이렉션 과정이 끝나면, 클라이언트의 세션 객체에는 최종 서비스(`msi.mju.ac.kr`)에서 사용 가능한 유효한 세션 쿠키(예: `JSESSIONID`)가 저장됩니다.
    -   만약 로그인 과정 중 서버로부터 에러 메시지(예: `alert('비밀번호가 틀렸습니다')`)가 반환되면, 이를 파싱하여 로그인 실패로 처리해야 합니다.
...
-   **Client Action**: 먼저 MSI 서비스의 메인 페이지에 `GET` 요청을 보냅니다.
    -   **Method**: `GET`
...
-   **Client Action (Parsing)**: 응답 HTML에서 CSRF 토큰을 추출합니다. 이 토큰은 `<meta name="_csrf" content="...">` 또는 `<input type="hidden" name="_csrf" value="...">` 형태로 존재합니다.

---

#### **2단계: 학생카드 페이지 접근 요청**

-   **Client Action**: 실제 웹사이트에서 사용자가 '학생카드' 메뉴를 클릭했을 때 발생하는 요청을 시뮬레이션합니다. 이는 일반적인 페이지 이동이 아닌, 특정 데이터를 포함한 `POST` 요청입니다.
    -   **Method**: `POST`
...
-   **Client Action**: 클라이언트는 응답 HTML에 비밀번호 입력 필드(`tfpassword`)가 있는지 확인하고, 확인되면 비밀번호를 서버로 보내 2차 인증을 수행합니다.
    -   **Method**: `POST`
...
-   **Client Action**: 이전 단계의 응답으로 받은 HTML에 포함된 자동 제출 폼을 처리합니다.
    ```html
...
-   **Client Action**: MSI 서비스의 메인 페이지에서 CSRF 토큰을 획득합니다. 이 과정은 학생카드 조회 시의 1단계와 동일합니다.
    -   **Method**: `GET`
    -   **URL**: `https://msi.mju.ac.kr/servlet/security/MySecurityStart`
...
-   **Client Action**: '학적변동내역' 메뉴 클릭에 해당하는 요청을 시뮬레이션합니다.
    -   **Method**: `POST`
...
-   **Client Action**: 응답받은 HTML 페이지를 파싱하여 학적변동내역 정보를 추출합니다.
    -   학생카드 파싱과 유사하게, `flex-table-item` 클래스를 가진 `div`들을 순회하며 '학번', '성명', '학적상태' 등의 항목명과 값을 추출합니다.
    -   추출된 정보로부터 학적변동내역을 구성합니다.

## 3. 결론

명지대학교 SSO와 MSI 시스템의 인증 과정은 복잡한 클라이언트-서버 통신 과정을 정교하게 재현해야만 자동화할 수 있습니다. 핵심은 다음과 같습니다.

1.  **정확한 순서 준수**: 각 단계의 요청은 이전 단계의 응답(쿠키, CSRF 토큰, 리다이렉션 URL 등)에 의존하므로, 정해진 순서를 반드시 따라야 합니다.
2.  **JavaScript 동작 모방**: 실제 브라우저에서 실행되는 JavaScript의 동작(암호화, 폼 제출, 리다이렉션)을 서버와의 HTTP 요청/응답만으로 시뮬레이션해야 합니다.
3.  **상태 관리**: 세션을 통해 여러 도메인(`sso.mju.ac.kr`, `msi.mju.ac.kr`)에 걸친 쿠키를 관리하여 로그인 상태를 유지해야 합니다.
4.  **동적 파싱**: 서버 응답이 변경될 수 있으므로, HTML 구조에 의존하는 파싱 로직(CSRF 토큰, 폼 데이터, 학생 정보 추출)은 유연하게 대처해야 합니다.

이 문서를 통해 명지대학교 웹 서비스와 어떻게 상호작용하는지에 대한 깊이 있는 이해를 얻을 수 있기를 바랍니다.